...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < sort_comp->n_levels && pid != 0; i++) {
        ...
        for (int n = 0; n < get_number_parts(i, sort_comp->n_levels); n++) {
            ...
            SEND: if (mq_send(cola, (char *) &msg[n], sizeof(msg[n]), 1) == -1) {
                ...
                /* Si durante el estado de bloqueo recibe una señal, vuelve a tratar de enviar */
        ...
        while(boolean == FALSE) {
            ...
            for (int n = 0; n < get_number_parts(i, sort_comp->n_levels); n++) {
                sem_wait(sem2);
                /* Si una tarea esta incompleta el padre ejecutara el bucle hasta que no lo este */
    ...
    /* El padre es el único en hacer unlink */
    if (pid != 0) {
        free(hijos);
        mq_unlink(COLA_MEN);
        shm_unlink(VAR_AUX);
